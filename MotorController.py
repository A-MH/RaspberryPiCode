...
default_pwm = 30
pwm_acceleration = 0.02 * default_pwm
pwm_deceleration = 0.02 * default_pwm

en_values = {"front": 4, "back": 27, "left": 9, "right": 5}

pwm_pins = {"front": None, "back": None, "left": None, "right": None}

in_values = {"front": [14, 17], "back": [15, 18], "left": [25, 11], "right": [8, 7]}
...
def setup():
    ...
    for key, value in in_values.items():
        ...
        pwm_pins[key] = GPIO.PWM(en_values[key], 100)
...
#                 is_decelerating = True
        if (old_travel_direction == "forward"):
            ...
            if is_new_direction == True:
                ...
                is_new_direction = False
            deviation0.append(deviation[1])
            deviation2.append(deviation[3])
            GPIO.output(in_values["front"][0], GPIO.HIGH)
            GPIO.output(in_values["back"][0], GPIO.HIGH)
...
def destroy():
    ...
    plt.plot(x_axis, diff_array, 'k', x_axis, deviation0, 'b', x_axis, deviation2, 'r')
